...
public class PredicatesProcessor extends BotConfigurationElementProcessor
{
    ...
    public String process(Element element, BotsConfigurationFileParser parser) throws ProcessorException
    {
         ...
         */
        if ("predicates".equals(element.getNodeName()) && "".equals(element.getAttribute("href"))) {
            ...
            parser.getCore().getBot(parser.getCurrentBot().getID()).setPredicatesFiles(path);

            // Add it to the AIMLWatcher, if active.
            if (parser.getCore().getSettings().useWatcher()) {
                parser.getCore().getPredicatesWatcher().addWatchFile(path);
            }
        }
        return EMPTY_STRING;
    }
