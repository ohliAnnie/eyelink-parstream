...
public class SubstitutionsProcessor extends BotConfigurationElementProcessor {
    ...
    public String process(Element element, BotsConfigurationFileParser parser) throws ProcessorException {
         ...
         */
        if ("substitutions".equals(element.getNodeName()) && "".equals(element.getAttribute("href"))) {
            ...
            parser.getCore().getBot(parser.getCurrentBot().getID()).setSubstitutionsFiles(path);

            // Add it to the AIMLWatcher, if active.
            if (parser.getCore().getSettings().useWatcher()) {
                parser.getCore().getSubstitutionsWatcher().addWatchFile(path);
            }
        }
        return EMPTY_STRING;
    }
