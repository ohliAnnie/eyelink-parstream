        ...
        if (!name.equals(EMPTY_STRING) && !value.equals(EMPTY_STRING))
        {
            ...
            try
            {
                if (PatternArbiter.matches(parser.getCore().getPredicateMaster().get(name, parser.getUserID(), parser.getBotID()), value, true))
                {
                	if (aimlLogger.isDebugEnabled())
                    {
                        aimlLogger.debug("Matched condition : \"" + value + "\"");
                    }
            ...
            {
                logger.error("ConditionProcessor got a non-AIML pattern in a value attribute.", e);
                return EMPTY_STRING;
            }
            return EMPTY_STRING;
        }
                                ...
                                case NAME_VALUE:
                                    ...
                                    try
                                    {
                                                ...
                                                livalue, true))
                                        {
                                        	if (aimlLogger.isDebugEnabled())
                                            {
                                                aimlLogger.debug("Matched <li>-condition: \"" + livalue + "\"");
                                            }
                                    ...
                                    {
                                        Logger.getLogger("programd").error(
