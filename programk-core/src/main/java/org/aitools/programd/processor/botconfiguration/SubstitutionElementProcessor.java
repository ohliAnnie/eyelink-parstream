...
abstract public class SubstitutionElementProcessor extends BotConfigurationElementProcessor
{
     ...
     * @throws org.aitools.programd.processor.ProcessorException if there is some problem processing
     */
    protected void addSubstitutions(SubstitutionType type, Element element, BotsConfigurationFileParser parser) throws ProcessorException
    {
        ...
        for (Element substitution : substitutions)
        {
            ...
            {
            	/**
            	 * @author sspark 2012.08.27 (ss.park@kt.com)
            	 * for korean matching
            	 */
            	//pattern = Pattern.compile(find, Pattern.CANON_EQ | Pattern.CASE_INSENSITIVE | Pattern.UNICODE_CASE);
                pattern = Pattern.compile(find, Pattern.CASE_INSENSITIVE | Pattern.UNICODE_CASE);
            }
