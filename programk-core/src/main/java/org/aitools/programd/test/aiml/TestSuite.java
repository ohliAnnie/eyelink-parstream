...
public class TestSuite implements Iterable<TestCase>
{
    ...
    public boolean run(String botid)
    {
        ...
        {
            this.multiplexor.getResponse(this.clearInput, this.TESTER, botid);
        }
        ...
        {
            boolean caseSuccessful = testCase.run(this.multiplexor, this.TESTER, botid);
            try {
            	//실제 상황을 가정함. input 간 간격을 둠.
