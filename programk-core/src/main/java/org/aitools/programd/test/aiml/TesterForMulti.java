...
public class TesterForMulti
{
     ...
     */
    public TesterForMulti(Core coreToUse, Logger testLogger, List<URL> suitePaths, URL testReports)
    {
        this.core = coreToUse;
    ...
    public String run(String botid, String suite, int runCount)
    {
        ...
        /* this.suites = loadTestsForMulti(this.suiteURLs, URLTools.contextualize(FileManager.getRootPath(), TEST_CASES_SCHEMA_LOCATION), this.multiplexor, this.logger); */
        this.suites = loadTestsForMulti(this.suiteURLs, this.core.getSettings().getSchemaLocationTestCases(), this.multiplexor, this.logger);
        if (null == botid)
        {
        ...
        //report.write(reportPath);  //sspark
    ...
    private void runAllSuites(String botid, int runCount)
    {
    	...
    	while (runCount-- > 0)
        {
            ...
            {
            	suite.setUserid(String.valueOf(suite.hashCode()));
            	new multiTester(suite, botid).start() ;
            }
        }
    }
    ...
    //sspark
    private static HashMap<String, TestSuite> loadTestsForMulti(List<URL> suiteList, URL schema, Multiplexor multiplexorToUse, Logger logger)
    {
        HashMap<String, TestSuite> suites = new HashMap<String, TestSuite>();
        for (URL path : suiteList)
        {
            ...
            for(int i = 0; i< TESTCNT; i++) {
            	TestSuite suite = TestSuite.load(path, schema, multiplexorToUse, logger);
            	suites.put(suite.getName()+String.valueOf(i), suite);
            }
        }
        return suites;
    }
    ...
    //sspark
    class multiTester extends Thread {
    	
    	TestSuite suite ;
    	String botId ;
    	
    	public multiTester(TestSuite suite , String botId) {
    		this.suite  = suite;
    		this.botId = botId; 
    	}
    	@Override
