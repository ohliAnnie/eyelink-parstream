...
abstract public class AbstractNodemaster implements Nodemapper
{
	 ...
	 *@since 2012.08.23   
	 **/
    protected AimlMap<String, Object> hidden;
    //old code
    ...
    /**
     * AimlMap object factory function.
     * @author sspark (ss.park@kt.com)
     * @since  2012.8.23
     * @return
     */
    protected AimlMap getAimlMapInstance() {
    	return new AimlMap<String, Object>() ;
     ...
     */
    @Deprecated
    protected boolean isMatch(String valueToMatch, String target) {
    	
    	boolean ret = valueToMatch.equalsIgnoreCase(target);
    	if(!ret) {
    		if(valueToMatch !=null && target !=null ){
    			if(!valueToMatch.startsWith("*") && !target.startsWith("*"))
    				ret = valueToMatch.matches(target);
