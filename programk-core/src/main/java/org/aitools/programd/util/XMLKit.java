...
public class XMLKit
{
    ...
    public static Document parseAsDocumentFragment(String text)
    {
        ...
        try
        {
        	...
        	DocumentBuilderFactory factory = DocumentBuilderFactory.newInstance();
            factory.setNamespaceAware(true);
            result =factory.newDocumentBuilder().parse(new InputSource(new StringReader(text))) ;
            /** old code  */
        ...
        /**  @author sspark 2012.08.23
         *   handling exception in factory.newDocumentBuilder()
         **/
        catch (ParserConfigurationException e) {
        	throw new DeveloperError("Parser Configuration Exception processing XML fragment: \"" + text + "\"", e);
		}
        return result;
    }
    ...
    /**
     * Replaces the CDATA[] tags with space
     * @param input the string on which to perform the replacement
     * @author sspark (ss.park@kt.com, 2012.08.23)
     * @return the string with entities replaced
     */
    public static String getCDATAText(String cdata)
    {
    	return cdata.replace(XMLKit.CDATA_START, "").replace(XMLKit.CDATA_END, "");
    }

    /**
     * Replaces the <star/>, <get/>... tags with predicate values
     *
     * @author sspark (ss.park@kt.com, 2012.08.23)
     * @param expression
     * @return
     * @throws org.aitools.programd.processor.ProcessorException
     */

    public static String transformPredicates(String expression, TemplateParser parser) throws ProcessorException
    {
       //하위 text element 를 가지지 않는 node만 처리
       //예) <star/>, <input/>, <bot name="..."/>, <get name="..."/>
       String patternTofind = "<.+?/>" ;  //예: <star index="1"/>, <get name="test"/>

       Pattern pattern =Pattern.compile(patternTofind);
       Matcher match = pattern.matcher(expression);
       String PredicateNode;
       String escapedValue;

       while(match.find())
       {
               PredicateNode = match.group().replaceAll("\\\\", "");  //<star index=\"1\"/> 에서 "\"을 삭제
            //javascript에서 특수문자로 인식되는 문자들(\"')에 대해 escape처리
            escapedValue= parser.processResponse(PredicateNode)
                                  .replaceAll("\\\\", "\\\\\\\\") //  \ -> \\  
                                  .replaceAll("\"", "\\\\\"")     //  " -> \"  
                                  .replaceAll("'", "\\\\'");      //  ' -> \'   
            expression = expression.replace(match.group(), escapedValue);
       }
        return expression;
    }
    ...
    public static String transformPredicates(String expression, TemplateParser parser) throws ProcessorException {
		   ...
		   while(match.find()) {
				...
				try {
					...
					document = builder.parse(new ByteArrayInputStream(PredicateNode.getBytes()));
					newele = document.getDocumentElement();
					//change xml-element with value
					expression = expression.replace(match.group(), parser.processElement(newele));
					
				} catch (ParserConfigurationException e) {
					throw new ProcessorException("Error- Parsing xml.", e);
				} catch (SAXException e) {
					throw new ProcessorException("Error- SAXException.", e);
				} catch (IOException e) {
					throw new ProcessorException("Error- IOException.", e);
