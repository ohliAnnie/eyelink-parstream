...
public class FlatFileMultiplexor extends Multiplexor {
    ...
    private Properties loadPredicates(String userid, String botid) {
        ...
        //file 존재 여부만 check 없을 경우 생성하지 않음.
        File predicateFile = FileManager.checkFile(fileName);
        if (predicateFile != null && predicateFile.canRead()) {
            try {
                //sspark load 실행 완료후에도 filestream이 닫히지 않음(java doc)
                FileInputStream fi = new FileInputStream(predicateFile);  //sspark
                predicates.load(fi);
                fi.close();  //sspark
            } catch (IOException e) {
    ...
    private void savePredicates(Properties predicates, String userid, String botid) {
        ...
        try {
            ...
            outputStream.close(); //sspark store 실행 완료후에도 filestream이 닫히지 않음(java doc)
        } catch (IOException e) {
    ...
    /**
     * @see org.aitools.programd.multiplexor.Multiplexor#useridCount(String)
     */
    @Override
