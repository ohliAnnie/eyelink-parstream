  ...
  var demo = new Vue({
    ...
    methods: {
      ...
      function generate(data, id, lineType, axisNum) {
        ...
        var legendSize = 10,
               legendColor = {'ampere':"#EDC951",  'voltage':"#CC333F", 'active_power':"#756bb1", 'apparent_power': "#31a354", 'reactive_power':"#fd8d3c", 'power_factor':"#00A0B0"};
       ...
       var line = d3.svg.line()
        ...
        path.selectAll(".path_line")
          .data(ddata)
          .enter()
          .append("path")             
          .attr("class", function (d) { return "path_line path_line_" + d['category']; })
          ...
          .attr("stroke",function (d) { return legendColor[d['category']]; });
...
/*       var points = svg.append("g")
        .attr("class", "path_point");

        points.selectAll(".path_circle")
        .data(ddata)
        .enter().append("circle")
        .attr("class", function (d) { return "path_circle path_circle_" + d['category']; })
...
/*          svgD['points'] = points;*/
      ...
      //inits chart
      self.lineFunc = new generate(data, "#LINE", "linear", 6);
    }
    },
    checkOpt: function (e) {
      var self = this;
      ...
      for (var i=0; i<self.lineCategory.length; i++) {
        if ($("#"+self.lineCategory[i]).prop("checked")) {
          ...
          d3.selectAll(".path_line_"+self.lineCategory[i]).transition().duration(300).style("display", 'inherit');
        }
        else
          d3.selectAll(".path_line_"+self.lineCategory[i]).transition().duration(300).style("display", 'none');
      }
