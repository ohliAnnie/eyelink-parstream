...
function displayCount() {
  ...
  var in_data = { url : "/dashboard/restapi/countDayEvent", type : "GET", data : data };
  ajaxTypeData(in_data, function(result){  
    if (result.rtnCode.code == "0000") {
       $('#event-count').text(result.today);               
       setStatus($('#event_status'), result.today/result.yday*100, 'day', result.yday); 
    }
  });

  var data = { date : day, gap : 'mon' };
  var in_data = { url : "/dashboard/restapi/countMonEvent", type : "GET", data : data };
  ajaxTypeData(in_data, function(result){  
    if (result.rtnCode.code == "0000") {    
      $('#mevent-count').text(result.tmon);        
      setStatus($('#mevent_status'), result.tmon/result.ymon*100, 'day', result.ymon); 
    }
  });
 
  var in_data = { url : "/dashboard/restapi/countFaultEvent", type : "GET", data : { date : day }};
  ajaxTypeData(in_data, function(result){            
    if (result.rtnCode.code == "0000") {      
       $('#fault-count').text(result.today);                       
       setStatus($('#fault_status'), result.today/result.yday*100, 'day', result.yday); 
    } 
  });         

  var in_data = { url : "/dashboard/restapi/sumActivePower", type : "POST", data : { date : day }};
  ajaxTypeData(in_data, function(result){            
    if (result.rtnCode.code == "0000") {
      console.log(result)
       $('#power').text(result.today.toFixed(1));                       
       setStatus($('#power_status'), result.today/result.yday*100, 'day', result.yday.toFixed(1)); 
    } 
  });           
}
