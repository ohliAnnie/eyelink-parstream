      ...
      displayLine: function() {
        ...
        function generate(data, id, lineType, axisNum) {
          ...
          path.selectAll(".click_line")
            ...
            .enter()
            .append("path")
            .attr("class", function (d) { return "click_line click_line_" + d['category']; })
...
function getNodePower(nodeList){
  ...
  var data = { startDate:sdate, endDate:edate, nodeId: node };
...
function clickNode(nodeId) {
  ...
  var in_data = { url : "/analysis/restapi/getClusterDetailRawData", type : "GET", data : data };  
  ajaxTypeData(in_data, function(result){
  $.ajax({
    if (result.rtnCode.code == "0000") {
      var data = result.rtnData;   
