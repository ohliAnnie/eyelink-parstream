...
/**
 * properties XML 파일 생성
 */
public class PropertiesTemplateProcess extends AbstractTemplateProcess {
    /**
     * The constant LOGGER.
     */
    private static final Logger LOGGER = LoggerFactory.getLogger(PropertiesTemplateProcess.class);
    ...
    public void execute(Exchange exchange) throws ApplicationException, BizCheckedException {
        ...
        CLogger.functionStart("PropertiesTemplateProcess.execute");

        //01. 해당 서비스의 StandBy 파일 명을 조회한다.
        BotFile deploy = botDeployService.findBotFile(body.getCpId(), AimlFileType.PROPERTIES.getValue(), body.getSubLabel());
        ...
        List<AimlProp> listAimlProp = botDeployService.findListAimlProp(body.getCpId());

        Map<String, Object> data = new HashMap<>();
        data.put("LIST", listAimlProp);

        isValidDirectory(deploy.getPath());
        writeFile(data, deploy.getPath() + "/" + deploy.getFileName(), "properties.ftl");
        if (body.getId() != 9999) {
            writeHistory(data,
                    deploy.getPath() + "/" + deploy.getFileName(),
                    "properties.ftl",
                    body.getId(),
                    AimlFileType.PROPERTIES.getValue(),
                    body.getUserId(),
                    body.getCpId(),
                    body.getSubLabel());
        }
        body.getFileMap().put("prop", deploy.getPath() + "/" + deploy.getFileName());
    ...
    public void clean(Exchange exchange) throws ApplicationException, BizCheckedException, IOException {
        ...
        try {
            ...
            deployHistory.setFileType(AimlFileType.PROPERTIES.getValue());
