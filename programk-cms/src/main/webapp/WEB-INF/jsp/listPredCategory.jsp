<%--
 Class Name  : listPredCategory.jsp
 Description : 콘텐츠 관리 - Predicates 카테고리 목록
...
<div class="searchGrpContent">
	...
	<div class="searchGrpContentBox content">
		<!--// 타이틀 및 위치안내 시작 -->
		<div class="titleBox">
			<h3 class="contentTitle">Predicates 카테고리 설정</h3>
			<p class="location">
				홈 > 콘텐츠 관리 > Predicates 관리 > 카테고리 설정
			</p>
		</div>
		<!-- 타이틀 및 위치안내 끝 //-->
		...
		<div class="txt mgnB7">
			<p>
				<span class="bul_txt">&#42;</span> Predicates 항목들을 카테고리로 묶어서 관리할 수 있습니다.<br/>
		...
		<div class="tableList">
			<table>
				<caption>Predicates 카테고리 설정-NO, 카테고리, 유형, 등록건수, 사용여부, 배포여부, 편집(수정, 삭제)</caption>
				<colgroup>
					<col class="w1 w5p">
					<col class="w2">
					<col class="w3 w10p">
...
<script type="text/javascript">
	...
	function fnDetailChat(cateId){
		location.href = "<c:url value='/listPred?cateId="+ cateId +"'/>";
	}
	...
	function fnEditLayer(type,id){
		$.ajax({    		
			url: "<c:url value='/detailPredCategory'/>",
	...
	function fnIsName(){
		...
		if(obj.find("#name").val() != ""){
			...
			$.ajax({    		
				url: "<c:url value='/isPredCategory'/>",
				data: formData, 
	        	type: 'POST', 
	            dataType: 'json', 
	        	success : function(response){  
	...
	function fnAction(){
		...
		if($("#id").val() == "0"){//등록
			url = "<c:url value='/addPredCategory'/>";
	    }else{//수정
	    	url = "<c:url value='/editPredCategory'/>";
	    }
	...
	function fnDelete(id,cpId){
	    ...
	    if (answer == true){
	    	$.ajax({    		
	        	url : "<c:url value='/deletePredCategory'/>",
