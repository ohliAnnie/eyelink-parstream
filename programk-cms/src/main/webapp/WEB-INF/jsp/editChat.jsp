<%--
 Class Name  : editChat.jsp
 Description : 콘텐츠 관리 - 대화 수정
...
<div class="searchGrpContent">
	...
	<div class="searchGrpContentBox user">
		<!--// 타이틀 및 위치안내 시작 -->
		<div class="titleBox">
			<h3 class="contentTitle">대화 수정</h3>
			<p class="location">
				홈 > 콘텐츠 관리 > 대화 관리 > 목록 > 대화 수정
			</p>
		</div>
		<!-- 타이틀 및 위치안내 끝 //-->
		...
		<div class="txt mgnB7">
			<p>
				...
				<span class="bul_txt">&#42;</span> 추천질문의 경우 추천질문으로 등록한 질문의 답변이 대화로 등록되어 있어야 합니다.<br/>
			</p>
		</div>
		<!-- 정보안내 끝 //-->
		...
		<input type="hidden" id="cateId" name="cateId" value="${results.cateId}" />
		...
		<div class="tableWriteBox">
			...
			<ul>
				...
				<li class="content">
					<input type="text" class="w200px" id="cateName" name="cateName" value="${results.cateName}" readonly/>
			...
			<ul>
				...
				<li class="content">
					<input type="text" class="w70p" id="input" name="input" maxlength="255" value="${fn:escapeXml(results.input)}"/>
					...
					<c:choose>
						<c:when test="${fn:length(images) > 0}">
							<c:forEach var="item" items="${images}" varStatus="status">
								<input type="text" class="w70p" id="imageUrl" name="imageUrl" maxlength="128" value="${item.url}"/>
								<a href="#" onclick="fnViewImageLayer();return false;" class="btnBg"><span class="icon_search"></span>미리보기</a>
							</c:forEach>
						</c:when>
						<c:otherwise>
							<input type="text" class="w70p" id="imageUrl" name="imageUrl" maxlength="128" />
								<a href="#" onclick="fnViewImageLayer();return false;" class="btnBg"><span class="icon_search"></span>미리보기</a>
						</c:otherwise>
					</c:choose>
				</li>
...
<!--		<ul>
				...
				<li class="content">
					<c:choose>
						<c:when test="${fn:length(images) > 0}">
							<c:forEach var="item" items="${images}" varStatus="status">
								<input type="text" class="w70p" id="imageAlt" name="imageAlt" maxlength="255" value="${item.alt}"/>
							</c:forEach>
						</c:when>
						<c:otherwise>
		...
		<div class="mindmap">
				...
				<li class="children__item">
					<div class="node">
						<div class="node__text">
							<c:if test="${results.thatId == ''}">
							<span id="thatInput">이전답변</span> 
							</c:if>
							<c:if test="${results.thatId != ''}">
							<span id="thatInput">${thatInput.input}</span> 
							</c:if> 
							<a href="#" onclick="fnAddThatLayer();return false;" class="btnBg"><span class="icon_plus"></span></a>
						</div>
						<input type="text" class="node__input" name="thatId" id="thatId" value="${results.thatId}">
					</div>
				</li>
			</ol>
			<div class="node node_root">
				<div class="node__text">${results.input}</div>
			...
			<ol class="children children_rightbranch">			
				<c:forEach var="item" items="${recommend}" varStatus="status">
					<li class="children__item">
						<div class="node">
							<div class="node__text">${fn:escapeXml(item.recommendInput)} <a href="#" onclick="fnDeleteRecommend(this);return false;" class="btnBg"><span class="icon_cancel"></span></a></div>
							<input type="text" class="node__input" name="tmp_recommendInput" value="${fn:escapeXml(item.recommendInput)}">
						</div>		
					</li>
				</c:forEach>		
				...
				<li class="content">
					...
					<c:choose>
						<c:when test="${fn:length(reply) > 0}">
							...
							<ul class="input">
								<li><input type="text" class="w400px" name="tmp_replyInput" maxlength="255" value="${fn:escapeXml(item.replyInput)}"/></li>
								<li class="btn">
									<c:if test="${status.index eq 0}">
									<a href="#" onclick="fnAddReplyOption();return false;" class="btnBg"><span class="icon_plus">추가</span></a>
									</c:if>
									<c:if test="${status.index ne 0}">
									<a href="javascript:;" class="icon_cancel" id="btnDelete">삭제</a>
									</c:if>
			...
			<ul>
				...
				<li class="content">
					<div class="memo">
						...
						<c:if test="${i eq item.seq}"><c:set var="optionVal" value="${item.val}"/></c:if>
						</c:forEach>
						<textarea style="height:50px;" id="option${i}" name="optionInput">${optionVal}</textarea>
					</div>
				</li>
			</ul>
			</c:forEach>
		...
		<div class="sub_table" id="addTestOption">
			<c:choose>
				<c:when test="${fn:length(test) > 0}">
					...
					<ul class="input">
						<li class="w96p"><input type="text" class="w100p" name="test" value="${fn:escapeXml(item.testInput)}" /></li>
						<li class="btn">
							<c:if test="${status.index eq 0}">
							<a href="#" onclick="fnAddTestOption();return false;" class="btnBg"><span class="icon_plus">추가</span></a>
							</c:if>
							<c:if test="${status.index ne 0}">
							<a href="javascript:;" class="icon_cancel" id="btnDelete">삭제</a>
							</c:if>
...
<script type="text/javascript">
	...
	$(document).ready(function(){
		...
		// 테스트 질문 삭제
        $("#addTestOption ul>li").find("#btnDelete").on('click', function(){
            $(this).parent().parent().remove();
        });
		
		// 텍스트 링크 삭제
        $("#addLinkOption ul>li").find("#btnDelete").on('click', function(){
            $(this).parent().parent().remove();
        });
		
     	// 추가 답변 삭제
        $("#addReplyOption ul>li").find("#btnDelete").on('click', function(){
            $(this).parent().parent().remove();
        });
	...
	function fnAddTestOption(){
        ...
        $("#addTestOption ul:last li:last").append($.btnDelete);
      	...
      	//데이터 초기화
        $("#addTestOption ul:last input").val("");
	...
	function fnAddLinkOption(){
        ...
        //데이터 초기화
        $("#addLinkOption ul:last input").val("");
	...
	function fnAddReplyOption(){
        ...
        $("#addReplyOption ul:last li:last").append($.btnDelete);
      	...
      	//데이터 초기화
        $("#addReplyOption ul:last input").val("");
	...
	function fnAddThatLayer(){
		$("#iframeThat").attr("src","/cms/searchChat?cpId="+$("#form #cpId").val()+"&inputId="+$("#form #id").val());
