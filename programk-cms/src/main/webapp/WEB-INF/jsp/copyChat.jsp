<%--
 Class Name  : copyChat.jsp
 Description : 콘텐츠 관리 - 대화 복사
...
<div class="searchGrpContent">
	...
	<div class="searchGrpContentBox user">
		<!--// 타이틀 및 위치안내 시작 -->
		<div class="titleBox">
			<h3 class="contentTitle">대화 복사</h3>
			<p class="location">
				홈 > 콘텐츠 관리 > 대화 관리 > 목록 > 대화 복사
			</p>
		</div>
		<!-- 타이틀 및 위치안내 끝 //-->
				...
				<li class="content">
					<select class="selectmenu" id="cpId" name="cpId">
						<c:forEach var="cp" items="${result}" varStatus="status">
							<option value="${cp.id}" ${cp.id == results.cpId ? 'selected="selected"' : '' }>${cp.label}</option>
						</c:forEach>				
					</select>
				</li>
			</ul>		
					...
					<c:choose>
						<c:when test="${fn:length(images) > 0}">
							<c:forEach var="item" items="${images}" varStatus="status">
								<input type="text" class="w70p" id="imageUrl" name="imageUrl" value="${item.url}"/>
								<a href="#" onclick="fnViewImageLayer();return false;" class="btnBg"><span class="icon_search"></span>미리보기</a>
							</c:forEach>
						</c:when>
						<c:otherwise>
							<input type="text" class="w70p" id="imageUrl" name="imageUrl"/>
								<a href="#" onclick="fnViewImageLayer();return false;" class="btnBg"><span class="icon_search"></span>미리보기</a>
						</c:otherwise>
					</c:choose>
				</li>
