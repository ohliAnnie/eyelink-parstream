<%--
 Class Name  : addProp.jsp
 Description : 콘텐츠 관리 - Properties 등록
...
<div class="searchGrpContent">
	...
	<div class="searchGrpContentBox user">
		<!--// 타이틀 및 위치안내 시작 -->
		<div class="titleBox">
			<h3 class="contentTitle">Properties 신규 생성</h3>
			<p class="location">
				홈 > 콘텐츠 관리 > Properties 관리 > 신규 생성
			</p>
		</div>
		<!-- 타이틀 및 위치안내 끝 //-->
		...
		<div class="txt mgnB7">
			<p>
				...
				1)등록 예시 - 이름 : username, 값 : 홍길동<br/>
				2)사용 예시 -  ${fn:escapeXml("<bot name=“username”/>")} 와 같이 사용됩니다. <br/>
		...
		<div class="tableWriteBox">
			...
			<ul>
				<li class="title">Properties 설정 <span class="required"></span></li>
				<li class="content">
					<div class="sub_table txtL" id="addOption">
						...
						<ul class="input">
							...
							<li><input type="text" id="tmp_val" name="tmp_val" maxlength="128" /></li>
...
<div class="popLayer categorySearchPop w600px">
	...
	<div class="popTopContentBox">
		<iframe id="iframeCategory" name="iframeCategory" src="<c:url value='/searchPropCategory'/>" width="580" height="240" marginwidth="0" marginheight="0" frameborder="no" scrolling="auto"></iframe>
	</div>
	<div class="bottomButton">
		<div class="right">
...
<script type="text/javascript">
	...
	function fnCategoryLayer(){
		$("#iframeCategory").attr("src","/cms/searchPropCategory?cpId="+$("#cpId").val());
	...
	function fnList(){
	    ...
	    if (answer == true){
	    	location.href = "<c:url value='/listProp'/>";
	    }
	}
	...
	function fnAdd(){
		if($('#form').valid()){
			...
			$("#val").val(arrtostr("tmp_val","text"));
			
			var formData = $('#form').serializeArray();
		    var answer = confirm("저장 하시겠습니까?");
		    if (answer == true){
		    	$.ajax({    		
		        	url : "<c:url value='/addProp'/>",
		        	data: formData, 
		        	type: 'POST', 
		            dataType: 'json', 
	...
	$('#form').validate({
		rules: {
			...
			tmp_val: {required:true,inputbox:true}
		},
		messages: {
			cateName: {
