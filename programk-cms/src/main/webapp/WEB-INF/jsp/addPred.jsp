<%--
 Class Name  : addPred.jsp
 Description : 콘텐츠 관리 - Predicates 등록
...
<div class="searchGrpContent">
	...
	<div class="searchGrpContentBox cp">
		<!--// 타이틀 및 위치안내 시작 -->
		<div class="titleBox">
			<h3 class="contentTitle">Predicates 신규 생성</h3>
			<p class="location">
				홈 > 콘텐츠 관리 > Predicates 관리 > 신규 생성
			</p>
		</div>
		<!-- 타이틀 및 위치안내 끝 //-->
		...
		<div class="txt mgnB7">
			<p>
				<span class="bul_txt">&#42;</span> 아래와 같이 사용됩니다.<br/>				
				1)등록 예시 - 이름 : username, 기본값 : 홍길동<br/>
				2)사용 예시 - 세팅할때<br/>
					&nbsp;&nbsp;&nbsp;-. ${fn:escapeXml("<set name=“username”>")}홍길동</set> 와 같이 설정해서 사용하며, 설정값이 없을 때는 기본값(default)이 설정됩니다.<br/>
					&nbsp;&nbsp;&nbsp;-. 동작 옵션이 Name이면 “username”이 표시되고, Value이면 “홍길동”이 표시됩니다.<br/>
				3)사용 예시 - 가져올때<br/>
					&nbsp;&nbsp;&nbsp;-. ${fn:escapeXml("<get name=“username”>")} 와 같이 사용합니다.<br/>
		...
		<div class="tableWriteBox">
			...
			<ul>
				<li class="title">Predicates 설정 <span class="required"></span></li>
				<li class="content">
					<div class="sub_table txtL" id="addOption">
						<ul>
							<li class="title">이름</li>
							<li class="title">기본값</li>
							<li class="title w15p">동작</li>
							<li class="title w4p"></li>
						</ul>
						<ul class="input">
							<li><input type="text" id="tmp_name" name="tmp_name" maxlength="128" /></li>
							<li><input type="text" id="tmp_basic" name="tmp_basic" maxlength="128" /></li>
							<li>
								<select class="w100p" id="tmp_val" name="tmp_val">
...
<div class="popLayer categorySearchPop w600px">
	...
	<div class="popTopContentBox">
		<iframe id="iframeCategory" name="iframeCategory" src="<c:url value='/searchPredCategory'/>" width="580" height="240" marginwidth="0" marginheight="0" frameborder="no" scrolling="auto"></iframe>
	</div>
	<div class="bottomButton">
		<div class="right">
...
<script type="text/javascript">
	...
	function fnCategoryLayer(){
		$("#iframeCategory").attr("src","/cms/searchPredCategory?cpId="+$("#cpId").val());
	...
	function fnList(){
	    ...
	    if (answer == true){
	    	location.href = "<c:url value='/listPred'/>";
	    }
	}
	...
	function fnAdd(){
		if($('#form').valid()){
			...
			//중복체크-이름
			if(unique("tmp_name","이름") != ""){
				alert(unique("tmp_name","이름"));
				return false;
			}			
			
			$("#name").val(arrtostr("tmp_name","text"));
			$("#basic").val(arrtostr("tmp_basic","text"));
			$("#val").val(arrtostr("tmp_val","select"));
			
			var formData = $('#form').serializeArray();
		    var answer = confirm("저장 하시겠습니까?");
		    if (answer == true){
		    	$.ajax({    		
		        	url : "<c:url value='/addPred'/>",
		        	data: formData, 
		        	type: 'POST', 
		            dataType: 'json', 
	...
	$('#form').validate({
		rules: {
			...
			tmp_name: {required:true,inputbox:true},
			tmp_basic: {required:true,inputbox:true}
		},
		messages: {
			cateName: {
