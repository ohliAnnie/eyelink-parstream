<%--
 ...
 Description : 콘텐츠 관리 - Predicates 목록
...
<div class="searchGrpContent">
	...
	<div class="searchGrpContentBox content">
		<!--// 타이틀 및 위치안내 시작 -->
		<div class="titleBox">
			<h3 class="contentTitle">Predicates</h3>
			<p class="location">
				홈 > 콘텐츠 관리 > Predicates 관리 > 목록
			</p>
		</div>
		<!-- 타이틀 및 위치안내 끝 //-->
		...
		<div class="txt mgnB7">
			<p>
				<span class="bul_txt">&#42;</span> BOT이 사용하는 글로벌한 변수값을 설정 할 수 있습니다.<br/>
				<span class="bul_txt">&#42;</span> ‘공용’으로 설정된 카테고리의 Predicates도 노출됩니다.<br/>
			</p>
		</div>
		<!-- 정보안내 끝 //-->
				...
				<ul>
					...
					<li class="content">
						<select class="selectmenu" id="searchType" name="searchType">
							...
							<option value="basic" ${'basic' == search.searchType ? 'selected="selected"' : '' }>기본값</option>
						</select>
		...
		<div class="tableList fixed">
			<table>
				<caption>Properties-NO, 카테고리, 이름(name), 기본값(default), 동작, 편집(수정, 삭제)</caption>
				<colgroup>
					<col class="w1 w5p">
					<col class="w2">
					<col class="w3">
...
<div class="popLayer writeCategorySet">
	...
	<div class="popTableBox">
		<div class="popTable">
			...
			<ul>
				<li class="title">기본값 <span class="required"></span></li>
				<li class="content">
					<input type="text" id="basic" name="basic"  maxlength="128"/>
				</li>
			</ul>
			<ul>
				<li class="title">동작</li>
				<li class="content">
					<select class="selectmenu w100p" id="val" name="val">
...
<script type="text/javascript">
	...
	function fnSearch(){
		...
		$("#formSearch").attr("action", "<c:url value='/listPred'/>");
		$("#formSearch").submit();
	}
	...
	function fnPage(page){		
		...
		$("#formPageing").attr("action", "<c:url value='/listPred'/>");
		$("#formPageing").submit();
	}

	//신규등록
	function fnAdd(){
		location.href = "<c:url value='/addPred'/>";
	}
	...
	function fnEditLayer(cateId,name){
		...
		$.ajax({    		
        	...
        	success : function(response){
        		...
        		$("#form #basic").val(basic);
        		$("#form #val").val(val);
        		$("#form #val").selectmenu("refresh");
	...
	function fnDownload(){
		...
		$("#formSearch").attr("action", "<c:url value='/downloadPred'/>");
		$("#formSearch").submit();
	}
	
	//샘플파일 다운로드
	function fnSampleDownload(){
		location.href = "<c:url value='/downloadSamplePred'/>";
	}
	
	//오류파일 다운로드
	function fnErrorDownload(){	
		$("#formUpload").attr("action", "<c:url value='/downloadErrorPred'/>");
		$("#formUpload").submit();
	}
