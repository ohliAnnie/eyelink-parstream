...
AnalysisProvider = function() {

}

// 단건에 대해서 Query를 수행한다.
AnalysisProvider.prototype.selectSingleQueryByID = function (queryId, datas, callback) {
  var vTimeStamp = Date.now();
  console.time('nodelib-Analysis/selectSingleQueryByID -> '+ queryId +' total ');
  console.log('nodelib-Analysis/selectSingleQueryByID -> (%s) queryID', queryId)
  ...
  parstream.connect(function(err) {
    if (err) {
      ...
      console.log('nodelib-Analysis/selectSingleQueryByID -> ' + sSql);

      console.time('nodelib-Analysis/selectSingleQueryByID -> ('+ queryId +') executeQuery');
      parstream.query(sSql, function(err, resultset) {
        console.timeEnd('nodelib-Analysis/selectSingleQueryByID -> ('+ queryId +') executeQuery');
        // console.log('nodelib-Analysis/selectSingleQueryByID -> resultset : %j', resultset);

        // console.log(err);
        // occur error
        var err = null;
        if (typeof resultset.error === 'string') {
          console.log('nodelib-Analysis/selectSingleQueryByID -> (%s) resultset error : %s', queryId, (typeof resultset.error === 'string'));
          err = resultset.error;
          console.timeEnd('nodelib-Analysis/selectSingleQueryByID -> '+ queryId +' total ');
          parstream.close();
          callback(resultset.error, [[]], datas);
        } else {
          console.log('nodelib-Analysis/selectSingleQueryByID -> (%s) resultset.rows.length : %d', queryId, resultset.rows.length);
          // console.log('nodelib-Analysis/selectSingleQueryByID -> resultset type : %s', (typeof resultset === 'object'));
          // console.log('nodelib-Analysis/selectSingleQueryByID -> resultset rows type : %s', (typeof resultset.rows === 'object'));
          console.timeEnd('nodelib-Analysis/selectSingleQueryByID -> '+ queryId +' total ');
          parstream.close();
