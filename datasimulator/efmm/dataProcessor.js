...
function processData(dirPath, file, nof, x) {
  ...
  var startDateTime = getDateTimeFromFileName(file);
  ...
  lineReader.on('line', function (line) {
    if ( rowsProcessed ) {  // 헤더 부분은 처리하지 않도록 하기 위한 조건
      ...
      var measure_time = DateUtil.getDate(startDateTime, CONSTS.DATEFORMAT.DATETIME,0,0,0,elapsedSeconds,'Y','Y');
      ...
      var indexDate = measure_time.substr(0,10).replace(/-/g,'.');
      ...
      if ( prevIndex == index ) {
        logger.trace('['+file+'] Pushing data into buffer....');
        jsonDataList.push(jsonData);
        if ( jsonDataList.length == bulkSize) {
          logger.debug('['+file+'] Saving ' + bulkSize + ' data..... on index \'' + index + '\'');
...
function indexSettings(index) {
  return {index: index,body: { index: { max_result_window: 100000 }}};
}
function saveBulkData(index, type, jsonDataList) {
  logger.trace('Inserting data into index: ',index);
  var in_data = {
    "index" : index,
    "type" : type,
    "body" : jsonDataList
...
function getDateTimeFromFileName(fileName) {
  // filename이 yyyyMMdd_HHmmss 형태
  var raw_date_time = fileName.split('.csv')[0].split('_');
  var date = raw_date_time[0];
  var time = raw_date_time[1];
